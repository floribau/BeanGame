...
public class Main {
        ...
        while (true) {
            ...
            if (stateID != 2 && ((startOpponent <= moveState && moveState <= endOpponent) || moveState == -1)) {
                if (moveState != -1) {
                    ...
                    System.out.println(
                        "Gegner waehlte: " + moveState + " /\t" + gameState.getTreasuryRed() + " - " + gameState.getTreasuryBlue());
                    System.out.println(gameState + "\n");
                }

                // apply minimax with alpha-beta-pruning
                System.out.println("Rufe Alpha-Beta-Search auf...");
                int selectField = AI.alphaBetaSearch(new State(gameState), redPlayer).field();

                updateBoard(gameState, selectField);
                System.out.println(
                    "Waehle Feld: " + (selectField + 1) + " /\t" + gameState.getTreasuryRed() + " - " + gameState.getTreasuryBlue());
                System.out.println(gameState + "\n\n");
    ...
    public static void switchRedTurn(State state) {
        state.setRedTurn(!state.isRedTurn());
        ;
    }

    public static void updateBoard(State state, int field) {
        ...
        if (board[field] == 2 || board[field] == 4 || board[field] == 6) {
            do {
                if (startField < 6) {
                    state.setTreasuryRed(state.getTreasuryRed() + board[field]);
                } else {
                    state.setTreasuryBlue(state.getTreasuryBlue() + board[field]);
                }
                board[field] = 0;
